    ...
    Mutation: {
        async createProduct(_, args, contextValue) {
            ...
            const collection = bucket
                ...
                .collection("products");
            const key = uuidv4();
            const createdMutationResult = await collection
                .insert(key, product)
                .catch((error) => {
                console.log(error);
                throw error; // Document not found
        ...
        async deleteProduct(_, args, contextValue) {
            ...
            const collection = bucket
                .scope("products-scope")
                .collection("products");
            const deleteMutationResult = await collection
                .remove(id)
                .catch((error) => {
                console.log(error);
                throw error; // Document not found
        ...
        async updateProduct(_, args, contextValue) {
            ...
            const collection = bucket
                .scope("products-scope")
                .collection("products");
            const updateMutationResult = await collection
                .replace(id, product)
                .catch((error) => {
                console.log(error);
                throw error; // Document not found
